            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden" style="width: 460px">
                <div class="p-8">
                    <heading :level="2" class="mb-6">
                        <template v-if="type == 'folder'">{{ __('Remove folder') }}: {{ name }}</template>
                        <template v-else>{{ __('Remove file') }}: {{ name }}</template>
                    </heading>
                    <template v-if="type == 'folder'">
                        <p class="text-80">{{__('Are you sure you want to remove this folder?')}}</p>
                        <p
                            class="text-sm text-80 mt-2"
                        >{{ __('Remember: The folder and all his contents will be delete from your storage') }}</p>
                    </template>
                    <template v-else>
                        <p class="text-80">{{__('Are you sure you want to remove this file?')}}</p>
                        <p
                            class="text-sm text-80 mt-2"
                        >{{ __('Remember: The file will be delete from your storage') }}</p>
                    </template>
                </div>
                ...
                <div class="bg-30 px-6 py-3 flex">
                    <div class="ml-auto">
                        ...
                        <button
                            ...
                            :class="{ 'cursor-not-allowed': isDeleting, 'opacity-50': isDeleting }"
                        >
